...
ULONG64 GetBeep()
{
    ...
    if (!EnumDeviceDrivers(Drivers, sizeof(Drivers), &BytesNeeded) || BytesNeeded > sizeof(Drivers))
        return 0;
    ...
    for (int i = 0; i < DriverCount; i++)
    {
        ...
        {
            if (_stricmp(DriverName, "beep.sys") == 0)
...
DWORD PeFileOffsetToRva(IMAGE_NT_HEADERS64* NtHeader, DWORD FileOffset)
{
